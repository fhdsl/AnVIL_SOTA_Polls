...
**Takeaway:** Of the ```r nrow(resultsTidy)``` responses, ```r nrow(resultsTidy %>% filter(UserType == "Returning User"))``` were returning users and ```r nrow(resultsTidy %>% filter(UserType == "Potential User"))``` were potential users. and ```r nrow(resultsTidy %>% filter(UserType == "Past and Potential User"))``` were Past and Potential Users. The majority of returning users belonged to the group who __ while the majority of potential users were __.
...
typeOfUserPlot <- resultsTidy %>%
  ...
  mutate(CurrentUsageDescription = case_when(
    str_detect(CurrentUsageDescription, "ongoing projects") ~ "For ongoing projects:\nconsistent project development\nand/or work",
    str_detect(CurrentUsageDescription, "long-term") ~ "For completed/long-term projects:\noccasional updates/maintenance\nas needed",
    str_detect(CurrentUsageDescription, "short-term projects") ~ "For short-term projects:\nshort, intense bursts\nseparated by a few months",
    str_detect(CurrentUsageDescription, "before") ~ "Used it before,\nbut don't currently plan to use it again",
    str_detect(tolower(CurrentUsageDescription), "never used") ~ "I have never\nused the AnVIL",
    str_detect(tolower(CurrentUsageDescription), "never heard") ~ "I have never\nheard of the AnVIL"
  )) %>%
  ggplot(aes(#x = count,
...
ggsave(here("plots/2025/respondent_usagedescription.png"), plot = typeOfUserPlot) #set plot size
```
...
degreePlot <- resultsTidy %>%
  ggplot(aes(y = fct_rev(fct_infreq(FurtherSimplifiedDegrees)),
             fill = UserType
             )) +
...
ggsave(here("plots/2025/degree_furthersimplified_usertype.png")) #set plot size
```
...
instTypePlot <- resultsTidy %>%
  mutate(FurtherSimplifiedInstitutionalType = factor(FurtherSimplifiedInstitutionalType,
...
ggsave(here("plots/2025/institutionalType_simplified_allResponses_colorUserType.png"), plot = instTypePlot) #set plot size
```
...
ggsave(here("plots/2025/tooldataresourcecomfortscore_singlepanel.png"), w = 2200, h = 1350, units = "px")
```
...
ggsave(here("plots/2025/typesOfData.png"), plot=everyone_type_data) #add plot size
```
...
ggsave(here("plots/2025/typesOfData_clinical.png"), plot=clinical_type_data)
```
...
ggsave(here("plots/2025/typesOfData_humangenomic.png"), plot=humangenomic_type_data)
```
...
ggsave(here("plots/2025/researchExperienceLevel_sequentialColor_noUserTypeSplit.png")) #set plot size
```
...
```{r}
onAnVILDF <- read_delim(here("data/2024/controlledAccessData_codebook.txt"), delim = "\t", col_select = c(whichControlledAccess, AnVIL_Availability))
```
...
ggsave(here("plots/2025/whichcontrolleddata.png"), plot = everyoneDataPlot) #add plot size
```
...
ggsave(here("plots/2025/whichcontrolleddata_clinical.png"), plot = clinicalDataPlot) #add plot size
```
...
ggsave(here("plots/2025/whichcontrolleddata_humangenomic.png"), plot = humanGenomicDataPlot) #add plot size
```
...
ggsave(here("plots/2025/whichcontrolleddata_nonhumangenomic.png"), plot = nonHumanGenomicDataPlot) #add plot size
```
...
totalRanksdf <-
  ...
  mutate(Feature =
           case_when(Feature == "TransparentBilling" ~ "Transparent billing\nability to foresee project cost",
                     Feature == "WorkgroupManagement" ~ "Easy workgroup and user management",
                     Feature == "Tools" ~ "Specific tools are available/supported",
                     Feature == "SupportDocs" ~ "On demand support and documentation",
                     Feature == "Data" ~ "Specific datasets are available/supported",
...
ggsave(here("plots/2025/dumbbellplot_xlim16_revaxis_rankfeatures.png"), plot = gdumbbell) #set plot size
...
ggsave(here("plots/2025/dumbbellplot_xlim15_revaxis_trainingmodalitypref.png"), plot = tdumbbell) #set plot size
```
...
```{r}
nonOtherResponses <- read_delim(here("data/2025/codebook.txt"), #read in the codebook
                                delim = "\t",
...
dmsPlot <- resultsTidy %>%
  ...
  tidyr::unite(RepositoriesMut, c("RepositoriesDMS", "suffix"), sep="") %>% #unite the responses with the suffix, appending the asterisk for user supplied answers
...
ggsave(here("plots/2025/DMS_Repositories.png"), plot = dmsPlot) #set save size
```
...
ggsave(here("plots/2025/fundingsources.png"), plot = fundingSourcePlot) #set save size
```
