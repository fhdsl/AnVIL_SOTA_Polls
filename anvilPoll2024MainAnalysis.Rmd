...
typeOfUserPlot <- resultsTidy %>%
  ...
  mutate(CurrentUsageDescription = case_when(
    ...
    CurrentUsageDescription == "I do not currently use the AnVIL, but have in the past" ~ "I do not currently use the AnVIL,\nbut have in the past",
...
ggsave(here("plots/2024/respondent_usagedescription.png"), plot = typeOfUserPlot) #set plot size
```
...
ggsave(here("plots/2024/degree_furthersimplified_usertype.png")) #set plot size
```
...
ggsave(here("plots/2024/institutionalType_simplified_allResponses_colorUserType.png"), plot = instTypePlot) #set plot size
```
...
ggsave(here("plots/2024/tooldataresourcecomfortscore_singlepanel.png"), w = 2200, h = 1350, units = "px")
```
...
ggsave(here("plots/2024/typesOfData.png"), plot=everyone_type_data) #add plot size
```
...
ggsave(here("plots/2024/typesOfData_clinical.png"), plot=clinical_type_data)
```
...
ggsave(here("plots/2024/typesOfData_humangenomic.png"), plot=humangenomic_type_data)
```
...
ggsave(here("plots/2024/researchExperienceLevel_sequentialColor_noUserTypeSplit.png")) #set plot size
```
...
ggsave(here("plots/2024/whichcontrolleddata.png"), plot = everyoneDataPlot) #add plot size
```
...
ggsave(here("plots/2024/whichcontrolleddata_clinical.png"), plot = clinicalDataPlot) #add plot size
```
...
ggsave(here("plots/2024/whichcontrolleddata_humangenomic.png"), plot = humanGenomicDataPlot) #add plot size
```
...
ggsave(here("plots/2024/whichcontrolleddata_nonhumangenomic.png"), plot = nonHumanGenomicDataPlot) #add plot size
```
...
ggsave(here("plots/2024/dumbbellplot_xlim16_revaxis_rankfeatures.png"), plot = gdumbbell) #set plot size
...
ggsave(here("plots/2024/dumbbellplot_xlim15_revaxis_trainingmodalitypref.png"), plot = tdumbbell) #set plot size
```
...
```{r}
nonOtherResponses <- read_delim(here("data/2024/codebook.txt"), #read in the codebook
                                delim = "\t",
...
ggsave(here("plots/2024/DMS_Repositories.png"), plot = dmsPlot) #set save size
```
...
ggsave(here("plots/2024/fundingsources.png"), plot = fundingSourcePlot) #set save size
```
